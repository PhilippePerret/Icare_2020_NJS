...
function rapportFinal(){
  var color = (()=>{
    if ( failure_count ) return "#E90000" // rouge
    else if ( pending_count ) return "orange"
    else return "#00DD00" // "#79EAAC" // vert
  })()
  var s_echec = failure_count > 1 ? 's' : ''
  var msg = `${success_count} succès - ${failure_count} échec${s_echec} - ${pending_count} en attente`
  Log.add("\n\n")
...
function failure(msg){
  failure_count ++
  Log.updateFailureCount(failure_count)
  Log.failure(msg)
}
function success(msg){
  success_count ++
  Log.updateSuccessCount(success_count)
  Log.success(msg)
}
