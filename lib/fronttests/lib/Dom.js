...
async function loadDom(){
  ...
  hasText(str, params){
    ...
    new Assertion(
      ...
      , this.positive
      , {
          pos_success: `La page contient bien « ${str} »${ajout}`
        , neg_success: `La page ne contient pas « ${str} »${ajout}`
        , pos_failure: `La page devrait contenir « ${str} »${ajout}`
        , neg_failure: `La page ne devrait pas contenir « ${str} »${ajout}`
        }
      ).evaluate(params)
    }
    ...
    async waitFor(selector){
      ...
      await Waits.waitForElementExists(selector)
    }
  }// /Dom
