...
async function loadDom(){
   ...
   */
  async searchForText(str, params){
    ...
    if ( params && params.in ){
      ...
      if ( allIns ) {
        ...
        // pass = false
        ajout += ' (INTROUVABLE)'
        return {pass:false, sujet:sujet, ajout:ajout}
      }
    } else {
    ...
    if ( pass === undefined ) {
      ...
      for ( conteneur of conteneurs ) {
        if ( conteneur.includes(str) ) {
          return {pass:true, sujet:sujet, ajout:ajout}
        }
      }
      return {pass:false, sujet:sujet, ajout:ajout}
    }
   ...
   */
  async hasText(str, params){
    var i = 0
    do {
      ++ i
      var res = await this.searchForText(str, params)
      console.log("res = ", res, str)
      if ( res.pass !== this.positive ) await wait(200)
    } while ( res.pass === false && i < 20 /* on essaie pendant 4 secondes */ )

    let {pass, ajout, sujet} = res
