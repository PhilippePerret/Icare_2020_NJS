...
class FTTMail {
  ...
  **/
  ...
  /*
    @method (dataMail) Mail.sent(dmail, options)
    @description Produit un succès si un mail répondant aux données +dmail+ a été transmis. Il faut avoir, au préalable, chargé le nombre de mails voulus avec `Mail.getLastMails(nombre)`. Et retourne les données du mail trouvé, if any.
    @provided
  ...
  static sent(dmail, options){
    return new MailSubject(dmail).evaluateExistence(options)
  }

  static getMails(filter){
    const my = this
    return new Promise(async (ok, ko) => {
      var ret = await Ajax.send({meth:'getMails',args: filter})
      my.allMails = JSON.parse(ret).map(dmail => new FTTMail(dmail))
      ok()
    })
  }
  static getLastMails(nombre){
    ...
    return new Promise((ok, ko) => {
      $.get(`/fttajax?meth=getLastMails&args[]=${nombre||1}`, function(ret){
        ...
        my.allMails = JSON.parse(ret).map(dmail => new FTTMail(dmail))
        // console.log("this.allMails : ", my.allMails)
        ok()
      })
    })
  }
  ...
  static getMailsWhoseMatch(dexpected) {
    const my = this
    var itMatches
    var reste = my.allMails.filter( lastMail => {
