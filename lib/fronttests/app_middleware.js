...
class FrontTests {
  ...
  **/
  static ajax(req,res){
    ...
    // res.send(`Mon message depuis /fttajax avec les paramètres : ${JSON.stringify(req.query)}`)
  }
  ...
  **/
  static getLastMails(nombre){
    ...
    fmails = fmails.splice(0,nombre)
    var ret = []
    for (var fmail of fmails){
      var nmail = thispath.basename(fmail)
      ret.push(`"${nmail}": ${thisfs.readFileSync(fmail)}`)
    }
    ret = `{${ret.join(', ')}}`
    return ret
  ...
  **/
  ...
  static buildSupportLoadingFile(){
    ...
    for ( var tfile of this.supportFileList ){
      ...
      console.log("Fichier : ", tfile)
      console.log("Fichier relatif : ", trelpath)
      code += `import("../__app_tests__/support${trelpath}")`
    }
    console.log("Code support final:\n", code)
