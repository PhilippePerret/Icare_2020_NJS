...
class FrontTests {
  ...
  **/
  ...
  static fileExists(params){
    var fullpath = thispath.join(APP_PATH, params.path)
    return thisfs.existsSync(fullpath)
  }
  ...
  /**
    Méthode ajax créant des inscriptions
  **/
  static createSignups(params){
    const Signup    = System.require('controllers/user/signup')
        , uuidv4    = require('uuid/v4')
        , AbsModule = System.require('models/AbsModule')

    params.nombre = params.nombre || 4
    const currentYear = new Date().getFullYear()
    while( params.nombre-- ) {
      var uuid = Math.rand(10000000)
      var token = uuidv4()
      var id    = token.replace(/\-/g,'').toUpperCase()
      var unew = new Signup({
          id:           id
        , pseudo:       `Pseudo${uuid}`
        , mail:         `mail${uuid}@chez.moi`
        , module:       1 + Math.rand(14)
        , naissance:    currentYear - Math.rand(70)
        , sexe:         ['F','H'][Math.rand(2)] || 'F'
        , token:        token
        , presentation: `fichier_presentation::uploads/presentation-${id}.md`
        , motivation:   `fichier_motivation::uploads/motivation-${id}.md`
      })
      unew.makeCandidatureFolder()
      unew.makeDataFile()
      // On crée les documents de présentation
      var pathSource        = System.pathFor('lib/fronttests/__app_tests__/support/documents/long_document.md')
      var pathPresentation  = System.pathFor(`uploads/presentation-${id}.md`)
      var pathMotivation    = System.pathFor(`uploads/motivation-${id}.md`)
      thisfs.copyFileSync(pathSource, pathPresentation)
      thisfs.copyFileSync(pathSource, pathMotivation)
    }// fin de boucle sur chaque candidature
  }
  /**
  ...
  **/
  ...
  /**
    Retourne les mails qui répondent aux critères +filter+
    @param {Object} filter
                      :after    Mails envoyés après ce temps
                      :to       Mails envoyés à ce destinataire
                      :from     Mails de cet expéditeur
  **/
  static getMails(filter){
    var fmails = glob.sync(`${this.folderMails}/*.json`)
    var ret = []
    for (var fmail of fmails){
      var nmail = thispath.basename(fmail)
      var mtime = parseInt(nmail.split('-')[0],10)
      if ( filter.after && mtime < filter.after  ) continue
      if ( filter.before && mtime > filter.before) continue
      var dmail = JSON.parse(thisfs.readFileSync(fmail))
      if ( filter.to && dmail['to'] != filter.to ) continue
      if ( filter.from && dmail['from'] != filter.from) continue
      if ( filter.subject && !dmail['subject'].match(filter.subject) ) continue
      // Si un mail arrive ici, c'est qu'il est bon
      ret.push(thisfs.readFileSync(fmail))
    }
    ret = `[${ret.join(', ')}]`
    return ret
  ...
  static getTickets(filter){
    var tickets = glob.sync(`${this.folderTickets}/*`)
    var ret = tickets
      .filter( ticket => {
        var time = parseInt(thispath.basename(ticket).split('-')[0],10)
        if ( filter.after && time < filter.after) return false
        if ( filter.before && time > filter.before) return false
        return true
      })
      .map( ticket => {
        var dticket = thispath.basename(ticket)
        dticket += '::'
        dticket += thisfs.readFileSync(ticket,'utf-8').replace(/"/g,'\\"')
        return dticket
      })
    ret = `["${ret.join('", "')}"]`
    return ret
  ...
  **/
  static reset(){
    ...
    FrontTests.emptyFolder(Icare.folderMails)

    // Vider le dossier des tickets
    FrontTests.emptyFolder(Icare.folderTickets)
  ...
  **/
  ...
  static get folderTickets(){
    return this._foldertickets || (this._foldertickets = this.config.folderTickets())
  }
