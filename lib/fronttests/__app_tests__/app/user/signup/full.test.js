...
export async function test(){
  ...
  //   Dom.hasText('Bravo !', {in:'h2'})
    ...
    action("On crée un bonne candidature en donnant toutes les bonnes données d'un coup")
    ...
    console.log("candidature_id = ", candidature_id)
    ...
    var themailconfirmationMail = Mail.sent({after: startTime, to:email, subject: 'Confirmation de votre adresse mèl'}, {success:`Le ${MAIL} de demande de confirmation de mail a été envoyé`})

    // Un ticket a été créé pour confirmer le mail
    await Ticket.getAll({after: startTime})
    var ticket = Ticket.exists({user_mail:email,code:"confirmMail",candidature_id:candidature_id})

    // TODO
    themailconfirmationMail.contains(`tck/${ticked.id}`)
